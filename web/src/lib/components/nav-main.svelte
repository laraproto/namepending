...
<script lang="ts">
	...
	import { hasPerm, type PermRequired } from '$lib/perm-utils';

	let { items }: NavMainProps = $props();

	const sidebar = Sidebar.useSidebar();

	let filteredItems = $derived.by(async () => {
					...
					? (
							await Promise.all(
								item.items.map(async (subItem) => {
									...
									return (await hasPerm(sidebar.user, subItem.permRequired)) ? subItem : null;
								})
							)
...
<Sidebar.Group>
	<Sidebar.Menu>
		{#await filteredItems then resolvedItems}
			{#each resolvedItems as item (item.title)}
