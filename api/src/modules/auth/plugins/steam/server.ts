...
export const steamOpenId = (options: SteamOpenIdPluginOptions) => {
	return {
		id: 'steam',
		endpoints: {
			getRedirectUrl: createAuthEndpoint(
				...
				async (ctx) => {
					const params = new URLSearchParams({
						...
						'openid.return_to': `${ctx.context.baseURL}/steam/link-callback`,
			...
			callback: createAuthEndpoint(
				...
				async (ctx) => {
					...
					if (!options.allowSignIn) {
						return ctx.redirect(`${options.failureRedirect}?error=signin_not_allowed`);
					}
					...
					return ctx.redirect(`${options.failureRedirect}?error=account_not_linked`);
				}
			)
		},
